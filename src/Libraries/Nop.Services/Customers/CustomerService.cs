...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(firstName))
            {
                query = query
                    ...
                    .Where((z => z.Attribute.KeyGroup == "Customer" &&
                        z.Attribute.Key == NopCustomersDefaults.FirstNameAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(lastName))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where((z => z.Attribute.KeyGroup == "Customer" &&
                        z.Attribute.Key == NopCustomersDefaults.LastNameAttribute &&
            ...
            if (dayOfBirth > 0 && monthOfBirth > 0)
            {
                ...
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where((z => z.Attribute.KeyGroup == "Customer" &&
                        z.Attribute.Key == NopCustomersDefaults.DateOfBirthAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(company))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where((z => z.Attribute.KeyGroup == "Customer" &&
                        z.Attribute.Key == NopCustomersDefaults.CompanyAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(phone))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where((z => z.Attribute.KeyGroup == "Customer" &&
                        z.Attribute.Key == NopCustomersDefaults.PhoneAttribute &&
            ...
            if (!string.IsNullOrWhiteSpace(zipPostalCode))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where((z => z.Attribute.KeyGroup == "Customer" &&
                        z.Attribute.Key == NopCustomersDefaults.ZipPostalCodeAttribute &&
        ...
        public virtual Customer InsertGuestCustomer()
        {
            ...
            //add to 'Guests' role
            var guestRole = GetCustomerRoleBySystemName(NopCustomersDefaults.GuestsRole);
            if (guestRole == null)
            ...
            bool clearPaymentMethod = true)
        {
            ...
            if (clearCouponCodes)
            {
                _genericAttributeService.SaveAttribute<string>(customer, NopCustomersDefaults.DiscountCouponCodeAttribute, null);
                _genericAttributeService.SaveAttribute<string>(customer, NopCustomersDefaults.GiftCardCouponCodesAttribute, null);
            }

            //clear checkout attributes
            if (clearCheckoutAttributes)
            {
                _genericAttributeService.SaveAttribute<string>(customer, NopCustomersDefaults.CheckoutAttributes, null, storeId);
            }

            //clear reward points flag
            if (clearRewardPoints)
            {
                _genericAttributeService.SaveAttribute(customer, NopCustomersDefaults.UseRewardPointsDuringCheckoutAttribute, false, storeId);
            }

            //clear selected shipping method
            if (clearShippingMethod)
            {
                _genericAttributeService.SaveAttribute<ShippingOption>(customer, NopCustomersDefaults.SelectedShippingOptionAttribute, null, storeId);
                _genericAttributeService.SaveAttribute<ShippingOption>(customer, NopCustomersDefaults.OfferedShippingOptionsAttribute, null, storeId);
                _genericAttributeService.SaveAttribute<PickupPoint>(customer, NopCustomersDefaults.SelectedPickupPointAttribute, null, storeId);
            }

            //clear selected payment method
            if (clearPaymentMethod)
            {
                _genericAttributeService.SaveAttribute<string>(customer, NopCustomersDefaults.SelectedPaymentMethodAttribute, null, storeId);
            }
        ...
        public virtual void DeleteCustomerRole(CustomerRole customerRole)
        {
            ...
            _cacheManager.RemoveByPattern(NopCustomerServicesDefaults.CustomerRolesPatternCacheKey);
        ...
        public virtual CustomerRole GetCustomerRoleBySystemName(string systemName)
        {
            ...
            var key = string.Format(NopCustomerServicesDefaults.CustomerRolesBySystemNameCacheKey, systemName);
        ...
        {
            var key = string.Format(NopCustomerServicesDefaults.CustomerRolesAllCacheKey, showHidden);
            return _cacheManager.Get(key, () =>
            {
                var query = from cr in _customerRoleRepository.Table
                            orderby cr.Name
