...
namespace Nop.Services.Plugins
{
    ...
    public class UploadService : IUploadService
    {
        ...
        protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
        {
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                var uploadedItemsFileEntry = archive.Entries
                    .FirstOrDefault(entry => entry.Name.Equals(NopPluginsDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
        ...
        protected virtual IDescriptor UploadSingleItem(string archivePath)
        {
            ...
            var pluginsDirectory = _fileProvider.MapPath(NopPluginsDefaults.Path);

            //get path to the themes directory
            var themesDirectory = string.Empty;
            if (!string.IsNullOrEmpty(NopPluginsDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(NopPluginsDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception($"The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{NopPluginsDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{NopPluginsDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{NopPluginsDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{NopPluginsDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(NopPluginsDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{NopPluginsDefaults.ThemeDescriptionFileName}";
        ...
        public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(NopPluginsDefaults.UploadsTempPath);
