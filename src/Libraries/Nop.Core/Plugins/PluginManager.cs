...
namespace Nop.Core.Plugins
{
    ...
    public class PluginManager
    {
        ...
        static PluginManager()
        {
            ...
            var refsPathName = _fileProvider.Combine(Environment.CurrentDirectory, NopPluginsDefaults.RefsPathName);
        ...
        private static IEnumerable<KeyValuePair<string, PluginDescriptor>> GetDescriptionFilesAndDescriptors(string pluginFolder)
        {
            ...
            foreach (var descriptionFile in _fileProvider.GetFiles(pluginFolder, NopPluginsDefaults.DescriptionFileName, false))
            {
        ...
        private static IList<string> GetInstalledPluginNames(string filePath)
        {
            ...
            if (!_fileProvider.FileExists(filePath))
            {
                ...
                filePath = _fileProvider.MapPath(NopPluginsDefaults.ObsoleteInstalledPluginsFilePath);
                if (!_fileProvider.FileExists(filePath))
                ...
                SaveInstalledPluginNames(pluginSystemNames, _fileProvider.MapPath(NopPluginsDefaults.InstalledPluginsFilePath));
        ...
        private static bool IsPackagePluginFolder(string folder)
        {
            ...
            if (!_fileProvider.GetDirectoryNameOnly(parent).Equals(NopPluginsDefaults.PathName, StringComparison.InvariantCultureIgnoreCase))
                return false;
        ...
        public static void Initialize(ApplicationPartManager applicationPartManager, NopConfig config)
        {
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                var pluginFolder = _fileProvider.MapPath(NopPluginsDefaults.Path);
                _shadowCopyFolder = _fileProvider.MapPath(NopPluginsDefaults.ShadowCopyPath);
                _reserveShadowCopyFolder = _fileProvider.Combine(_fileProvider.MapPath(NopPluginsDefaults.ShadowCopyPath), $"{NopPluginsDefaults.ReserveShadowCopyPathName}{DateTime.Now.ToFileTimeUtc()}");
                ...
                try
                {
                    var installedPluginSystemNames = GetInstalledPluginNames(_fileProvider.MapPath(NopPluginsDefaults.InstalledPluginsFilePath));
                    ...
                    if (config.ClearPluginShadowDirectoryOnStartup)
                    {
                        ...
                        foreach (var directory in _fileProvider.GetDirectories(_shadowCopyFolder, NopPluginsDefaults.ReserveShadowCopyPathNamePattern))
                        {
                            try
                            {
        ...
        public static void MarkPluginAsInstalled(string systemName)
        {
            ...
            var filePath = _fileProvider.MapPath(NopPluginsDefaults.InstalledPluginsFilePath);
        ...
        public static void SavePluginDescriptor(PluginDescriptor pluginDescriptor)
        {
            ...
            var filePath = _fileProvider.Combine(_fileProvider.GetDirectoryName(pluginDescriptor.OriginalAssemblyFile), NopPluginsDefaults.DescriptionFileName);
            if (!_fileProvider.FileExists(filePath))
